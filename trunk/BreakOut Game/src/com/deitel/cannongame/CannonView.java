   ...
   implements SurfaceHolder.Callback
{
   ...
   private Line[] target = new Line[NUM_TARGET_LINE]; // start and end points of the target
   private int[] targetDistance = new int[NUM_TARGET_LINE]; // target distance from top
   private int[] targetBeginning = new int[NUM_TARGET_LINE]; // target distance from left
   private double pieceLength; // length of a target piece
   private int[] targetEnd = new int[NUM_TARGET_LINE]; // target bottom's distance from left
   ...
   private boolean[][] hitStates = new boolean[NUM_TARGET_LINE][TARGET_PIECES]; // is each target piece hit?
   private int targetPiecesHit; // number of target pieces hit (out of NUJ_TARGET_LINE * TARGET_PIECES)
   ...
   public void newGame()
   {
      ...
      cannonball.x = (padBeginning + padEnd)/2; // align x-coordinate with cannon
      cannonball.y = padDistance-cannonballRadius-10; // centers ball vertically
     	 ...
     	 for(int i=0;i<NUM_TARGET_LINE;i++){
     		 if (cannonball.x + cannonballRadius >= target[i].start.x && 
       	            ...
       	            cannonball.y - cannonballRadius <= targetDistance[i])
       	         {
  	            	...
  	            	int section = 
  	            		(int) ((cannonball.x - target[i].start.x) / pieceLength);
     	               ...
     	               !hitStates[i][section])
     	            {
     	              ...
     	              cannonballVelocityY *= -1; // reverse the cannonball's Y direction
   ...
   public double alignCannon(MotionEvent event)
   {
      ...
      if (centerMinusX != 0) // prevent division by 0
         angle1 = Math.atan((double) (padDistance - touchPoint.y) / centerMinusX);
   ...
   public void movePad(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY)
   {
   ...
   public void drawGameElements(Canvas canvas)
   {
      ...
      canvas.drawText(getResources().getString(
         R.string.time_used_format, totalElapsedTime), 30, 50, textPaint);
      ...
      canvas.drawLine(pad.start.x, pad.start.y, pad.end.x, pad.end.y, padPaint);
      ...
      for(int i=0;i<NUM_TARGET_LINE;i++){
          ...
          for (int j = 0; j < TARGET_PIECES; j++)
          {
             ...
             if (!hitStates[i][j])
             {
                ...
                canvas.drawLine(currentPoint.x, currentPoint.y, (int) (currentPoint.x + pieceLength),
